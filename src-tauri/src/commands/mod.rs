...
pub fn delete_pi(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM memberships WHERE pi_id=?1", params![id])
        .map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM art_leadership WHERE pi_id=?1", params![id])
        .map_err(|e| e.to_string())?;
    conn.execute(
        "DELETE FROM solution_leadership WHERE pi_id=?1",
        params![id],
    )
    .map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM program_increments WHERE id=?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn activate_pi(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "UPDATE program_increments SET status='Planned' WHERE status='Active'",
        [],
    )
    .map_err(|e| e.to_string())?;
    conn.execute(
        "UPDATE program_increments SET status='Active' WHERE id=?1",
        params![id],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
...
pub fn complete_pi(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "UPDATE program_increments SET status='Completed' WHERE id=?1",
        params![id],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
...
pub fn get_solutions(db: State<Database>) -> Result<Vec<Solution>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT id, name, description FROM solutions ORDER BY name")
        .map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map([], |row| {
            Ok(Solution {
...
pub fn create_solution(
    ...
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO solutions (name,description) VALUES (?1,?2)",
...
pub fn get_arts(db: State<Database>, solution_id: Option<i64>) -> Result<Vec<Art>, String> {
    ...
    let (sql, p): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = match solution_id {
        Some(sid) => (
            "SELECT id, solution_id, name FROM arts WHERE solution_id=?1 ORDER BY name".into(),
            vec![Box::new(sid)],
        ),
        None => (
            "SELECT id, solution_id, name FROM arts ORDER BY name".into(),
            vec![],
        ),
    };
...
pub fn create_art(
    ...
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO arts (solution_id,name) VALUES (?1,?2)",
        params![solution_id, name],
    )
    .map_err(|e| e.to_string())?;
    Ok(conn.last_insert_rowid())
}
...
pub fn delete_art(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM arts WHERE id=?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn get_teams(db: State<Database>, art_id: Option<i64>) -> Result<Vec<ProductTeam>, String> {
    ...
    let (sql, p): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = match art_id {
        Some(aid) => (
            "SELECT id, art_id, name FROM product_teams WHERE art_id=?1 ORDER BY name".into(),
            vec![Box::new(aid)],
        ),
        None => (
            "SELECT id, art_id, name FROM product_teams ORDER BY name".into(),
            vec![],
        ),
    };
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
...
pub fn create_team(db: State<Database>, art_id: Option<i64>, name: String) -> Result<i64, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO product_teams (art_id,name) VALUES (?1,?2)",
        params![art_id, name],
    )
    .map_err(|e| e.to_string())?;
    Ok(conn.last_insert_rowid())
...
pub fn delete_team(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM product_teams WHERE id=?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn deactivate_person(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("UPDATE people SET active=0 WHERE id=?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn reactivate_person(db: State<Database>, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("UPDATE people SET active=1 WHERE id=?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub fn deactivate_person_remove_assignments(
    db: State<Database>,
    person_id: i64,
) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    conn.execute("UPDATE people SET active=0 WHERE id=?1", params![person_id])
        .map_err(|e| e.to_string())?;
    conn.execute(
    ...
    .map_err(|e| e.to_string())?;
    conn.execute(
        "DELETE FROM art_leadership WHERE person_id=?1",
        params![person_id],
    )
    .map_err(|e| e.to_string())?;
    conn.execute(
        "DELETE FROM solution_leadership WHERE person_id=?1",
        params![person_id],
    )
    .map_err(|e| e.to_string())?;
    Ok(())
...
pub fn get_person_total_fte(
    ...
    let m: i64 = conn
        .query_row(
            "SELECT COALESCE(SUM(fte_percent),0) FROM memberships WHERE person_id=?1 AND pi_id=?2",
            params![person_id, pi_id],
            |r| r.get(0),
        )
        .unwrap_or(0);
...
#[tauri::command]
pub fn get_person_allocations(
    db: State<Database>,
    person_id: i64,
    pi_id: i64,
) -> Result<Vec<PersonAllocation>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    ...
    let rows = stmt
        .query_map(params![person_id, pi_id], |row| {
            Ok(PersonAllocation {
                allocation_id: row.get(0)?,
                entity_id: row.get(1)?,
                role: row.get(2)?,
                fte_percent: row.get(3)?,
                entity: row.get(4)?,
                level: row.get(5)?,
            })
        })
        .map_err(|e| e.to_string())?;
    for r in rows {
        allocs.push(r.map_err(|e| e.to_string())?);
    }
...
pub fn get_team_members(
    ...
    team_id: i64,
    pi_id: i64,
) -> Result<Vec<MembershipWithPerson>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    ...
    let rows = stmt
        .query_map(params![team_id, pi_id], |row| {
            Ok(MembershipWithPerson {
                id: row.get(0)?,
                team_id: row.get(1)?,
                person_id: row.get(2)?,
                role: row.get(3)?,
                fte_percent: row.get(4)?,
                pi_id: row.get(5)?,
                full_name: row.get(6)?,
                email: row.get(7)?,
                active: row.get(8)?,
...
#[tauri::command]
pub fn get_solution_leadership(
    db: State<Database>,
    solution_id: i64,
    pi_id: i64,
) -> Result<Vec<LeadershipWithPerson>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare(
    ...
    let rows = stmt
        .query_map(params![solution_id, pi_id], |row| {
...
pub fn get_art_leadership(
    ...
    let rows = stmt
        .query_map(params![art_id, pi_id], |row| {
            Ok(LeadershipWithPerson {
                id: row.get(0)?,
                entity_id: row.get(1)?,
                person_id: row.get(2)?,
...
pub fn get_conflicts(db: State<Database>, pi_id: i64) -> Result<Vec<ConflictPerson>, String> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id,full_name,email,department FROM people WHERE active=1 ORDER BY full_name",
        )
        .map_err(|e| e.to_string())?;
    let people: Vec<(i64, String, Option<String>, Option<String>)> = stmt
        .query_map([], |row| {
    ...
    for (pid, name, email, dept) in people {
        ...
        if total > 100 {
            ...
            let rows = astmt
                .query_map(params![pid, pi_id], |row| {
                    Ok(PersonAllocation {
                        allocation_id: row.get(0)?,
                        entity_id: row.get(1)?,
                        role: row.get(2)?,
...
#[tauri::command]
pub fn get_solution_fte_breakdown(
    db: State<Database>,
    pi_id: i64,
) -> Result<Vec<SolutionFTEBreakdown>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT id, name FROM solutions ORDER BY name")
        .map_err(|e| e.to_string())?;
    let solutions: Vec<(i64, String)> = stmt
...
pub fn get_role_distribution(db: State<Database>, pi_id: i64) -> Result<RoleDistribution, String> {
    ...
    let team: i64 = conn
        .query_row(
            "SELECT COALESCE(SUM(fte_percent),0) FROM memberships WHERE pi_id=?1",
            params![pi_id],
            |r| r.get(0),
        )
        .unwrap_or(0);
    let art: i64 = conn
        .query_row(
            "SELECT COALESCE(SUM(fte_percent),0) FROM art_leadership WHERE pi_id=?1",
            params![pi_id],
            |r| r.get(0),
        )
        .unwrap_or(0);
    let sol: i64 = conn
        .query_row(
            "SELECT COALESCE(SUM(fte_percent),0) FROM solution_leadership WHERE pi_id=?1",
            params![pi_id],
            |r| r.get(0),
        )
        .unwrap_or(0);
...
#[tauri::command]
pub fn get_cost_rates(
    db: State<Database>,
    hub_filter: Option<String>,
    dept_filter: Option<String>,
) -> Result<Vec<CostRate>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn get_cost_rate_hubs(db: State<Database>) -> Result<Vec<String>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT DISTINCT hub_code FROM cost_rates ORDER BY hub_code")
        .map_err(|e| e.to_string())?;
    let rows = stmt
...
pub fn get_cost_rate_depts(db: State<Database>) -> Result<Vec<String>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT DISTINCT department FROM cost_rates ORDER BY department")
        .map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map([], |row| row.get(0))
        .map_err(|e| e.to_string())?;
...
pub fn search_all(db: State<Database>, term: String) -> Result<SearchResults, String> {
    ...
    let teams: Vec<TeamWithArt> = stmt
        .query_map(params![like], |row| {
            Ok(TeamWithArt {
                id: row.get(0)?,
                art_id: row.get(1)?,
    ...
    let arts: Vec<ArtWithSolution> = stmt
        .query_map(params![like], |row| {
            Ok(ArtWithSolution {
                id: row.get(0)?,
                solution_id: row.get(1)?,
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, description FROM solutions WHERE name LIKE ?1 LIMIT 5")
        .map_err(|e| e.to_string())?;
    let solutions: Vec<Solution> = stmt
        .query_map(params![like], |row| {
            Ok(Solution {
                id: row.get(0)?,
                name: row.get(1)?,
...
pub fn execute_csv_import(
    ...
    let mut field_to_idx: std::collections::HashMap<String, usize> =
    ...
    for row in &rows {
        ...
        if person_id.is_none() && !full_name.is_empty() {
            if let Ok(id) = conn.query_row(
                "SELECT id FROM people WHERE full_name=?1",
                params![full_name],
                |r| r.get::<_, i64>(0),
            ) {
                person_id = Some(id);
        ...
        if person_id.is_none() && !full_name.is_empty() {
            ...
            let dept_opt = if department.is_empty() {
                None
            } else {
                Some(department.as_str())
            };
            let cc_opt = if cost_center.is_empty() {
                None
            } else {
                Some(cost_center.as_str())
            };
            let hub_opt = if hub.is_empty() {
                None
            } else {
                Some(hub.as_str())
            };
            let co_opt = if company.is_empty() {
                None
            } else {
                Some(company.as_str())
            };
        ...
        if !department.is_empty() {
            conn.execute(
                "UPDATE people SET department=?1 WHERE id=?2",
                params![department, pid],
            )
            .ok();
        }
        if !hub.is_empty() {
            conn.execute("UPDATE people SET hub=?1 WHERE id=?2", params![hub, pid])
                .ok();
        }
        if !company.is_empty() {
            conn.execute(
                "UPDATE people SET company=?1 WHERE id=?2",
                params![company, pid],
            )
            .ok();
        }
        ...
        if !team_name.is_empty() && !art_name.is_empty() {
            let sol_id: i64 = if !solution_name.is_empty() {
                match conn.query_row(
                    "SELECT id FROM solutions WHERE name=?1",
                    ...
                    Err(_) => {
                        conn.execute(
                            "INSERT INTO solutions (name) VALUES (?1)",
                            params![solution_name],
                        )
                        .map_err(|e| e.to_string())?;
                ...
                match conn.query_row("SELECT id FROM solutions LIMIT 1", [], |r| {
                    r.get::<_, i64>(0)
                }) {
                    Ok(id) => id,
                    Err(_) => continue,
            ...
            let art_id: i64 = match conn.query_row(
                "SELECT id FROM arts WHERE name=?1 AND solution_id=?2",
                params![art_name, sol_id],
                |r| r.get::<_, i64>(0),
            ) {
                Ok(id) => id,
                Err(_) => {
            ...
            let team_id: i64 = match conn.query_row(
                "SELECT id FROM product_teams WHERE name=?1 AND art_id=?2",
                params![team_name, art_id],
                |r| r.get::<_, i64>(0),
            ) {
                Ok(id) => id,
                Err(_) => {
...
pub fn export_pi_as_csv(db: State<Database>, pi_id: i64) -> Result<String, String> {
    ...
    let mut csv_out =
        "Name,Email,Department,Hub,Cost Center,Company,Team,ART,Solution,Role,FTE%\n".to_string();
    let rows = stmt
        .query_map(params![pi_id], |row| {
            Ok((
                row.get::<_, String>(0)?,
    ...
    for result in rows {
        let (name, email, dept, hub, cc, company, team, art, sol, role, fte) =
...
#[tauri::command]
pub fn switch_database(app: AppHandle, db: State<Database>, db_path: String) -> Result<(), String> {
    let conn = crate::db::open_database(&db_path);
    db.replace_connection(conn);
