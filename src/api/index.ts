...
import type {
  ...
  LeadershipWithPerson,
  PersonAllocation,
  ConflictPerson,
  RoleDistribution,
  SolutionFTEBreakdown,
  CostRate,
  SearchResults,
  CsvParseResult,
  ImportMapping,
...
// ARTs
export const getArts = (solution_id?: number) => invoke<Art[]>('get_arts', { solutionId: solution_id ?? null });
...
// Teams
export const getTeams = (art_id?: number) => invoke<ProductTeam[]>('get_teams', { artId: art_id ?? null });
...
// People
export const getPeople = (active_only?: boolean) => invoke<Person[]>('get_people', { activeOnly: active_only ?? null });
export const getPersonById = (id: number) => invoke<Person | null>('get_person_by_id', { id });
...
export const createMembership = (
  ...
  invoke<number>('create_membership', {
    teamId: team_id,
    personId: person_id,
    role,
...
export const createLeadership = (
  ...
  invoke<number>('create_leadership', {
    level,
    entityId: entity_id,
    personId: person_id,
    role,
...
export const deleteLeadership = (level: string, id: number) => invoke<void>('delete_leadership', { level, id });
...
// Dashboard
export const getConflicts = (pi_id: number) => invoke<ConflictPerson[]>('get_conflicts', { piId: pi_id });
...
export const switchDatabase = (dbPath: string) => invoke<void>('switch_database', { dbPath });
