          ...
          {conflicts.length === 0 ? (
            ...
            conflicts.map((c, i) => (
              <tr key={c.id}>
                ...
                <td>{c.department || '—'}</td>
                <td>
                  <span className="pct-badge pct-badge--danger">{c.total_fte}%</span>
                </td>
                <td>
                  {c.assignments} role{c.assignments > 1 ? 's' : ''}
                </td>
                <td>
                  <button className="btn btn-sm btn-primary" onClick={() => handleResolve(c.id)}>
                    Resolve
                  </button>
                </td>
              </tr>
