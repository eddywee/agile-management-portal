...
export function PiManagementTab() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (
      !confirm(
        `Delete PI "${name}"? All memberships and leadership assignments scoped to this PI will be permanently removed.`,
      )
    )
      return;
    ...
    <>
      <div style={{ padding: '0 32px 16px', display: 'flex', justifyContent: 'flex-end' }}>
        <button
          ...
          onClick={() => showModal('New Program Increment', <AddPIForm onDone={refresh} />, null)}
        >
          ...
          <tbody>
            {allPIs.map((pi) => (
              <tr key={pi.id}>
                ...
                <td>
                  <span
                    className={`badge badge--${pi.status === 'Active' ? 'green' : pi.status === 'Planned' ? 'accent' : 'dim'}`}
                  >
                    {pi.status}
                  </span>
                </td>
                <td style={{ display: 'flex', gap: 4 }}>
                  <button
                    ...
                    onClick={() => showModal('Edit PI', <EditPIForm pi={pi} onDone={refresh} />, null)}
                  >
                    Edit
                  </button>
                  {pi.status === 'Planned' && (
                    <button className="btn btn-sm btn-outline" onClick={() => handleActivate(pi.id)}>
                      Activate
                    </button>
                  )}
                  {pi.status === 'Active' && (
                    <button className="btn btn-sm btn-outline" onClick={() => handleComplete(pi.id)}>
                      Complete
                    </button>
                  )}
                  <button
                    ...
                    onClick={() => handleDelete(pi.id, pi.pi_name)}
                  >
                    Delete
                  </button>
                </td>
