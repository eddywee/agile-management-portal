  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 12 }}>
            <select
              ...
              style={{ maxWidth: 160, padding: '8px 28px 8px 12px' }}
              value={hubFilter}
            ...
            <select
              className="form-select"
              style={{ maxWidth: 180, padding: '8px 28px 8px 12px' }}
              value={deptFilter}
      ...
      <div className="data-table-wrap" style={{ paddingTop: 0 }}>
            ...
            {rates.length ? (
              rates.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.department}</td>
                  <td>{r.role_level}</td>
                  <td style={{ textAlign: 'right', fontFamily: 'monospace', color: 'var(--white)' }}>
                    €{r.daily_rate.toFixed(2)}
                  </td>
                  <td>
                    <span className="badge badge--accent">{r.effective_pi}</span>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={5} style={{ textAlign: 'center', color: 'var(--dim)', padding: 24 }}>
                  No cost rates match filters
                </td>
              </tr>
            )}
          </tbody>
        ...
        >
          <span>
            Showing <span style={{ color: 'var(--white)', fontWeight: 600 }}>{rates.length}</span> rates
          </span>
        </div>
      </div>
    </>
