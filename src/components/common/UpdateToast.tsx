...
export function UpdateToast() {
  ...
  useEffect(() => {
    if (!toastVisible) { setExiting(false); return; }
  ...
  return (
    <div className={`toast${exiting ? ' toast--exit' : ''}`}>
      <div className="toast__content">
        ...
        <span>Update Available &mdash; <strong>v{update.version}</strong></span>
      </div>
      <button className="toast__action" onClick={handleDetails}>View Details</button>
    </div>
  );
}
