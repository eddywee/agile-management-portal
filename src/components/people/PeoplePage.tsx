...
export function PeoplePage() {
  ...
  const loadPeople = useCallback(async () => {
    ...
    if (activePI) {
      ...
      await Promise.all(
        all.map(async (p) => {
          fteMap[p.id] = await api.getPersonTotalFTE(p.id, activePI.id);
        }),
      );
      setFtes(fteMap);
    }
          ...
          <tbody>
            {filtered.map((p, i) => {
              ...
              return (
                <tr
                  key={p.id}
                  style={{ opacity: p.active ? 1 : 0.5, cursor: 'pointer' }}
    ...
    <div className="person-detail-panel">
      <div className="detail-header">
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          ...
          <div>
            <h3>
              {person.full_name}{' '}
              {!person.active && (
                <span className="badge badge--dim" style={{ fontSize: 11, verticalAlign: 'middle' }}>
                  Inactive
                </span>
              )}
            </h3>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {person.active ? (
            ...
            <button className="btn btn-sm btn-outline" onClick={handleReactivate}>
              Reactivate
            </button>
          )}
      ...
      <div className="detail-section">
        <div className="detail-section__title">
          ...
          <span className={totalCls} style={{ fontSize: 14 }}>
            ...
            <tbody>
              {allocs.map((a, i) => (
                <tr key={i}>
                  ...
                  <td>{a.role}</td>
                  <td>
                    <span className="fte-bar">
                      <span className="fte-bar__fill fte-bar__fill--ok" style={{ width: `${a.fte_percent}%` }} />
                    </span>
                    {a.fte_percent}%
                  </td>
                  <td style={{ whiteSpace: 'nowrap' }}>
                    ...
                    <button className="icon-btn" title="Remove" onClick={() => handleRemoveAlloc(a)}>
                      <svg viewBox="0 0 14 14" fill="none">
                        <path
                          d="M2.5 4h9M5 4V2.5h4V4M3.5 4v7.5h7V4"
                          stroke="currentColor"
                          strokeWidth="1.1"
                          strokeLinecap="round"
                          strokeLinejoin="round"
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <button className="btn btn-sm btn-outline" onClick={handleAddToTeam}>
            + Add to Team
          </button>
          <button className="btn btn-sm btn-outline" onClick={handleAddArtLeadership}>
            + ART Leadership
          </button>
          <button className="btn btn-sm btn-outline" onClick={handleAddSolLeadership}>
            + Solution Leadership
          </button>
        </div>
      </div>
              ...
              <div key={h.pi.id} className={`pi-timeline-item${h.pi.id === piId ? ' active' : ''}`}>
                ...
                <div className="pi-timeline-content">
                  <div className="pi-timeline-label">
                    {h.pi.pi_name}
                    <span
                      className={`badge badge--${h.pi.status === 'Active' ? 'green' : h.pi.status === 'Planned' ? 'accent' : 'dim'}`}
                      style={{ fontSize: 8, padding: '1px 6px', marginLeft: 4 }}
                    >
                      {h.pi.status}
                    </span>
                  </div>
                  <div className="pi-timeline-detail">
                    <span className={h.total > 100 ? 'alloc-over' : 'alloc-ok'}>{h.total}%</span> across {h.count}{' '}
                    assignment{h.count !== 1 ? 's' : ''}
                  </div>
                </div>
              </div>
            ))}
